...
public class ODataETagTestCase extends DSSIntegrationTest {
    ...
    public void validateETagConcurrentHandlingTestCaseForPutMethod() throws Exception {
        ...
        headers.remove("If-None-Match");
        ODataRequestThreadExecutor threadExecutor = new ODataRequestThreadExecutor("PUT", content, new ConcurrentHashMap<>(headers), endpoint);
        ...
        Assert.assertEquals(response[0], ODataTestUtils.OK);
        headers.remove("If-Match");
        threadExecutor = new ODataRequestThreadExecutor("DELETE", null, new ConcurrentHashMap<>(headers), endpoint);
        threadExecutor.start();
        threadExecutor.join();
        headers.put("If-Match", etag);
        content = "{\"TYPE\" : \"MB\"}";
