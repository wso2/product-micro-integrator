...
public class WsdlResource extends APIResource {
    ...
    private String buildServiceWsdlUrl(org.apache.axis2.context.MessageContext axisMsgCtx, String serviceName) {
        String host = deriveHost(axisMsgCtx);
        int port = deriveHttpPort();
        String protocol = "http";
        return protocol + "://" + host + ":" + port + "/services/" + serviceName + "?wsdl";
    }
    ...
    private String deriveHost(org.apache.axis2.context.MessageContext axisMsgCtx) {
        ...
        if (axisMsgCtx != null && axisMsgCtx.getConfigurationContext() != null &&
            ...
            Object val = axisMsgCtx.getConfigurationContext().getAxisConfiguration().getParameter("hostname")
                    .getValue();
            if (val != null) {
                host = val.toString();
            }
    ...
    private int deriveHttpPort() {
        String portStr = CarbonServerConfigurationService.getInstance()
                .getFirstProperty(CarbonServerConfigurationService.HTTP_PORT);
        int port = 8290;
        try {
            if (portStr != null) {
